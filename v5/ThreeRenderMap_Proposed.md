...
### ❌ **Sprite Scaling Test Results (9/12/25):**
- **Tested:** Simple billboard scaling with `tileSize` multiplier in `getBillboardSize()`
- **Implementation:** `'mountain': { width: 1.2 * tileScale, height: 1.5 * tileScale }`
- **Problem:** 2D sprites designed for flat map view appear "floating" when scaled as 3D billboards
- **Root cause:** Sprites lack proper 3D ground anchoring and perspective design
- **Attempted fix:** Reduced height scaling (0.6x) to minimize floating appearance
- **Result:** Still appears floating - sprites not designed for billboard presentation
- **Conclusion:** ✅ **ShapeForge 3D assets confirmed as better solution** - proper 3D models with automatic ground positioning
