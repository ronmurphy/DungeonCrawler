...
class InventoryManager {
     ...
     */
    getCurrentCharacter() {
        // First try the characterManager system
        ...
        // Fall back to global character object (used by main.js)
        if (window.character) {
    ...
    renderInventory() {
        ...
        inventory.forEach((item, index) => {
            // Skip items with missing names or invalid indices
            if (!item || !item.name || index === undefined) {
                console.warn('Skipping invalid inventory item:', item, 'at index:', index);
                return;
            }
            ...
            itemElement.innerHTML = `
                <div class="item-header">
                    ...
                    <div class="item-actions">
                        ...
                        ${(item.value || 0) > 0 ? `<button class="item-action-btn sell-btn" onclick="window.inventoryManager.sellItem(${index})" title="Sell for ${sellValue} GP">💰</button>` : ''}
                        <button class="item-action-btn remove-btn" onclick="window.inventoryManager.removeItem(${index})" title="Remove">🗑️</button>
                    </div>
                </div>
                <div class="item-details">
                    <div class="item-type-badge">${item.type || 'misc'}</div>
                    ${item.damage ? `<div class="item-stat">⚔️ Damage: ${item.damage}</div>` : ''}
                    ${item.defense ? `<div class="item-stat">🛡️ Defense: +${item.defense}</div>` : ''}
                    ${(item.value || 0) > 0 ? `<div class="item-value">💰 ${item.value} GP</div>` : ''}
    ...
    /**
     * Update gold display in DCC modal header
     */
    updateDCCModalGoldDisplay() {
        const goldElement = document.getElementById('dcc-modal-player-gold');
        if (!goldElement) return;
        
        const currentChar = this.getCurrentCharacter();
        const playerGold = currentChar?.gold || 0;
        goldElement.textContent = playerGold.toLocaleString();
    }
     ...
     */
    renderDCCItemsModal() {
        // Update player gold display in modal header
        this.updateDCCModalGoldDisplay();
        ...
        itemsToShow.forEach(item => {
            ...
            itemElement.innerHTML = `
                ...
                <div class="dcc-modal-item-value ${!canAfford ? 'insufficient-gold' : 'clickable-price'}" 
                     onclick="${canAfford ? `window.inventoryManager.purchaseDCCItem(${JSON.stringify(item).replace(/"/g, '&quot;')})` : ''}"
                     title="${canAfford ? `Click to purchase for ${itemCost} GP` : `Insufficient gold (need ${itemCost} GP)`}">
                    💰 ${item.value || 0} GP
                </div>
            `;
