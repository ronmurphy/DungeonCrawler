...
if (!window.ShaderEffectsManager) {
  class ShaderEffectsManager {
     ...
     * @param {Object} renderer - Reference to the ThreeMapRenderer
     */
    constructor(renderer) {
      ...
      this.maxTempEffects = this.isMobile ? 5 : 15; // Fewer effects on mobile
      this.maxActiveEffects = this.isMobile ? 3 : 8; // Limit concurrent effects
      this.effectDistance = this.isMobile ? 25 : 50; // Shorter range on mobile
      
      // Auto-detect quality based on device capabilities
      this.qualityLevel = this.detectQualityLevel();
      this.enabled = this.qualityLevel !== 'disabled';
      this.debug = false;
      this.effectCleanupInterval = null;
      
      console.log(`🎨 ShaderEffectsManager: ${this.enabled ? 'enabled' : 'disabled'} (quality: ${this.qualityLevel}, mobile: ${this.isMobile})`);
     ...
     */
    detectQualityLevel() {
      // Disable shaders entirely on very low-end devices
      if (this.isMobile && navigator.hardwareConcurrency && navigator.hardwareConcurrency < 4) {
        return 'disabled';
      }
      ...
      if (this.isMobile) {
        return maxTextures >= 8 && maxVaryings >= 8 ? 'low' : 'disabled';
      } else {
        if (maxTextures >= 16 && maxVaryings >= 16) return 'medium';
        if (maxTextures >= 8 && maxVaryings >= 8) return 'low';
        return 'disabled';
      }
    }
...
registerEffectDefinitions() {
  ...
  // 🐉 Custom Dragon Effect for Dragon Tiles
  this.registerEffectType('dragon', {
    keywords: ['dragon', 'wyrm', 'drake', 'dragonfire'],
    color: 0xff4400, // Dragon orange-red
    intensity: 1.5,
    distance: this.isMobile ? 6 : 10,
    decay: 2,
    particleCount: this.isMobile ? 8 : 15,
    particleSize: 0.06,
    particleColor: 0xff6622,
    animationSpeed: 1.2,
    scale: 1.0,
    create: this.createDragonEffect.bind(this)
  });
...
getObjectSize(object) {
  ...
  return defaultSize;
    ...
    applyEffect(object, effectType, qualityOverride = null) {
      ...
      try {
        // Use quality override if provided, otherwise use default quality level
        const effectiveQuality = qualityOverride || this.qualityLevel;
        
        // Call the create function from the definition with the effective quality
        const effectData = definition.create(object, definition, effectiveQuality);
  ...
  return {
    ...
    update: function(deltaTime) {
      ...
      if (this.renderer && this.renderer.camera) {
        magicPlane.lookAt(this.renderer.camera.position);
        backPlane.lookAt(this.renderer.camera.position);
      }
...
createPropGlowEffect(prop, options = {}) {
   ...
   */
    ...
    removeEffect(objectId) {
      ...
      try {
        ...
        if (effectData.light && !effectData.container) {
          this.scene.remove(effectData.light);
        }

        if (effectData.particles && !effectData.container) {
          this.scene.remove(effectData.particles);
        }
      ...
      if (effectData.type === 'waterProp') {
        ...
        if (effectData.material && effectData.material.uniforms && effectData.material.uniforms.time) {
          ...
          if (this.renderer && this.renderer.renderer) {
            // ThreeMapRenderer doesn't use needsRender flag
            // Rendering happens automatically in the render loop
          }
        }
    ...
    /**
     * 🐉 Create Dragon Effect - Custom effect for dragon tiles
     * Combines fire particles with dragonic energy swirls
     * @param {Object3D} object - The tile object to apply the effect to
     * @param {Object} definition - Effect definition
     * @param {string} qualityLevel - Quality level setting
     * @returns {Object} Effect data
     */
    createDragonEffect(object, definition, qualityLevel = 'medium') {
      console.log(`🐉 Creating dragon effect - enabled: ${this.enabled}, quality: ${this.qualityLevel}`);
      
      if (!this.enabled || this.qualityLevel === 'disabled') {
        console.log(`🐉❌ Dragon effect disabled`);
        return null;
      }

      console.log(`🐉🎯 Creating dragon effect at position:`, object.position);

      // Create container for dragon effects
      const container = new THREE.Group();
      container.position.copy(object.position);
      container.position.y += 0.3; // Raise effect above tile

      // Add to scene (using this.scene since we adapted it for ThreeMapRenderer)
      this.scene.add(container);
      console.log(`🐉✅ Added dragon container to scene`);

      // 🔥 Dragon Fire Light
      const light = new THREE.PointLight(
        definition.color,
        definition.intensity * 0.8, // Slightly dimmer for mobile
        definition.distance,
        definition.decay
      );
      light.position.y += 0.4;
      container.add(light);
      console.log(`🐉💡 Added dragon light`);

      let particles = null;
      let dragonAura = null;

      // Only add particles on medium quality and above
      if (qualityLevel !== 'low') {
        // 🔥 Fire Particles
        const particleCount = this.isMobile ? 8 : definition.particleCount;
        ...
        // 🐉 Dragon Aura (swirling energy effect)
        const auraGeometry = new THREE.RingGeometry(0.3, 0.8, 16);
        const auraMaterial = new THREE.ShaderMaterial({
          uniforms: {
            time: { value: 0 },
            color: { value: new THREE.Color(definition.color) }
          },
          vertexShader: `
            varying vec2 vUv;
            void main() {
          ...
          fragmentShader: `
            ...
            void main() {
              ...
              float spiral = sin(angle * 3.0 + time * 2.0 + dist * 8.0) * 0.5 + 0.5;
              
              // Fade from center to edge
              float fadeOut = 1.0 - smoothstep(0.2, 0.4, dist);
              float fadeIn = smoothstep(0.1, 0.2, dist);
              float alpha = spiral * fadeOut * fadeIn * 0.6;
        ...
        dragonAura = new THREE.Mesh(auraGeometry, auraMaterial);
        dragonAura.rotation.x = -Math.PI / 2; // Lay flat on ground
        container.add(dragonAura);
      }

      // Return effect data for animation and cleanup
      return {
        container,
        light,
        particles,
